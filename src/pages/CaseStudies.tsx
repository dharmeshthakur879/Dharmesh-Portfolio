...
import { Search, FileText, Sparkles, X, CheckCircle, BarChart3, TrendingUp, Layers } from "lucide-react";
    ...
    <div className="pt-28 pb-24 px-6 max-w-7xl mx-auto min-h-screen flex flex-col justify-between">
      <div>
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          >
            A curated collection of <span className="text-white font-semibold">{CASE_STUDIES.length} real-world digital marketing audit decks and strategy reports</span>. Click any case study card to view the standalone PDF report directly.
          </motion.p>
          ...
          >
            <div className="flex flex-col items-center p-2">
              <span className="text-xl sm:text-2xl font-mono font-bold text-[#D4A853]">{CASE_STUDIES.length}</span>
              <span className="text-[11px] font-mono text-text-sec uppercase tracking-wider">PDF Reports</span>
            </div>
            <div className="flex flex-col items-center p-2 border-l border-[#D4A853]/15">
              ...
              <span className="text-[11px] font-mono text-text-sec uppercase tracking-wider">Data-Driven</span>
            </div>
            <div className="flex flex-col items-center p-2 border-l border-[#D4A853]/15">
              <span className="text-xl sm:text-2xl font-mono font-bold text-white">8+</span>
              <span className="text-[11px] font-mono text-text-sec uppercase tracking-wider">Core Domains</span>
            </div>
            <div className="flex flex-col items-center p-2 border-l border-[#D4A853]/15">
              <span className="text-xl sm:text-2xl font-mono font-bold text-[#D4A853]">Standalone</span>
              <span className="text-[11px] font-mono text-text-sec uppercase tracking-wider">PDF Viewer</span>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mb-10 space-y-6">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-[#0A0E17]/80 p-4 rounded-2xl border border-[#D4A853]/20 backdrop-blur-md shadow-md">
            ...
            <div className="relative w-full md:w-96">
              ...
              <input
                type="text"
                placeholder="Search case studies by brand, topic, or keyword..."
                value={searchQuery}
            ...
            <div className="flex items-center space-x-3 text-xs font-mono text-text-sec w-full md:w-auto justify-between md:justify-end">
              <span>
                Showing <strong className="text-[#D4A853]">{filteredCaseStudies.length}</strong> of{" "}
        ...
        <AnimatePresence mode="wait">
          {filteredCaseStudies.length > 0 ? (
            ...
            >
              ...
              <h3 className="text-lg font-display font-bold text-white">No Case Studies Found</h3>
              <p className="text-xs text-text-sec mt-1 max-w-md mx-auto">
                No report matched "{searchQuery}" in category "{selectedCategory}". Try adjusting your query or resetting filters.
              </p>
              <button
                onClick={() => {
                  setSelectedCategory("All");
      ...
      <div className="mt-20 pt-6 border-t border-[#D4A853]/15 text-center text-xs font-mono text-text-sec/60 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center space-x-2">
          ...
          <span>Dynamically structured in single data array (<code className="text-white">src/data/caseStudies.ts</code>)</span>
        </div>
        <div className="text-[11px]">
          Click any card or button to launch the PDF report in a new tab.
        </div>
      </div>
    </div>
  );
